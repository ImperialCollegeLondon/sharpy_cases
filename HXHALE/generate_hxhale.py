#!/usr/env python
# Hinged X-HALE (HX-HALE) model for SHARPy.
# Version 2.0 sharpy multibody code
# For original see IFASD 2019 paper by: del Carre, A and Teixeira, P and Palacios, R and Cesnik, C. E. S.
# 
# Original XHALE by Alfonso del Carre, June 2019
# HX-HALE by Gregor Wilson, March 2024
# ============================================================================
...
data['1'] = {
   ...
   'gust_shape': '1-cos',  # '1-cos' for vertical gust
}
...
for case in cases:
    ...
    if vertical_tail:
        case_name = 'hxhale'
        print('Generating hxhale with vertical Ctail')
    else:
        case_name = 'hxhale'
        print('Generating hxhale with horizontal Ctail')
    ...
    if vertical_tail:
        alpha = 7.6178*np.pi/180                    # angle of attack [rad]
        beta = 0.*np.pi/180                         # sideslip angle  [rad]
        cs_deflection = -4.1232*np.pi/180           # elevators deflection [rad]
        aileron_deflection = 0*np.pi/180            # aileron deflection [rad]
        thrustC = 0.2410                            # baseline thrust [N]
        ...
        in_structural_twist = 0*np.pi/180 #was 5 degrees for the original XHALE, removed bec
        #in_structural_twist = 0*np.pi/180
    ...
    # point [0, 0, 0].
    #space_offset = 50 * chord / 8
    space_offset = 300*chord/8
    ...
    # geometrical data
    span_section = 1.0
    #dihedral_outer = 10*np.pi/180 #is 10 degrees for the standard XHALE
    dihedral_outer = 0*np.pi/180 
    ...
    span_inner_winglet = 0.125
    span_hinged_winglet = 0.875
    ...
    n_elem_section = 4
    # structural elements in inner winglet
    n_elem_section_dihedral = 1
    # elements in hinged winglet
    n_elem_section_winglet = 7
    ...
    n_elem_fin = 1
    n_elem_main = int((n_sections-1)*n_elem_section + n_elem_section_dihedral + n_elem_section_winglet)
    # number of aero surfaces. To understand the logic of this, check SHARPy's
    # documentation
    #n_surfaces = 20
    n_surfaces = 20 #add 2 surfaces for the hinged winglets was 22 in total
    ...
    #MANUAL TSTEP OVERRIDE
    #n_tstep = 5

    # variable wake discretisation
    cfl1 = False            #Disable CFL condition to allow non-uniform panels
    dx1 = dt*u_inf          #Generate initial wake panels with enforced CFL condition
    ndx1 = 40               #Number of initial wake panels 
    r = 1.1                 #Wake panel growth rate after ndx1 panels                 
    dxmax = 50000*dx1          #Maximum wake panel length
    mstar_fact = 66         #Number of chordwise wake panels
    ...
    n_elem += n_elem_fin
    n_elem += n_elem_fin
    # n_elem += n_elem_fin # Centre fins removed due to some wake blowup present at centreline, could add back by uncommenting the relevant lines
    # n_elem += n_elem_fin
...
# number of nodes per part
    ...
    n_node_section_winglet = n_elem_section_winglet*(n_node_elem - 1) + 1
...
# total number of nodes
    ...
    end_nodesL = np.zeros((n_sections+1,), dtype=int)
    end_nodesR = np.zeros((n_sections+1,), dtype=int)  # ADDED +1 HERE TO INCLUDE THE HINGED WINGLET IN THE END NODES DEFINITION
    end_elementsL = np.zeros((n_sections+1,), dtype=int)
    end_elementsR = np.zeros((n_sections+1,), dtype=int)
    ...
    sweep = np.zeros((n_elem, n_node_elem)) #ADDED FOR HINGE FLARE ANGLE BUT THE AERO GENERATOR IS NOT SET UP TO SWEEP PANELS GENERALLY.
...
# FUNCTIONS-------------------------------------------------------------
    ...
    def read_beam_data(filename=route + '/inputs/beam_properties.xlsx'):
        ...
        # mass
        # mass_sheet = pd.read_excel(filename, sheet_name='mass', header=1, skiprows=1, index_col=0)
        mass_sheet = pd.read_excel(filename, sheet_name='mass', header=1, index_col=0)
        # remove units
        ...
        # stiffness
        # stiff_sheet = pd.read_excel(filename, sheet_name='stiffness', header=1, skiprows=1, index_col=0)
        stiff_sheet = pd.read_excel(filename, sheet_name='stiffness', header=1, index_col=0)
        # remove units
    ...
    def read_lumped_mass_data(filename=route + '/inputs/lumped_mass_symmetric.xlsx'):
        import pandas as pd
    ...
    def generate_fem():
        ...
        body_number[we:we + n_elem_section] = 0
        ...
        body_number[we:we + n_elem_section_dihedral] = 0
        ...
        #SECTION HINGED WINGLET R

        beam_number[we:we + n_elem_section_winglet] = 3
        body_number[we:we + n_elem_section_winglet] = 1
        structural_twist[we:we + n_elem_section_winglet, :] = in_structural_twist
        y[wn:wn + n_node_section_winglet] = y[wn - 1] + np.linspace(0.0,
                                                                np.cos(dihedral_outer)*span_hinged_winglet,
                                                                n_node_section_winglet)
        z[wn:wn + n_node_section_winglet] = z[wn - 1] + np.linspace(0.0,
                                                                np.sin(dihedral_outer)*span_hinged_winglet,
                                                                n_node_section_winglet)
        for ielem in range(n_elem_section_winglet):
            conn[we + ielem, :] = ((np.ones((3, ))*(we + ielem)*(n_node_elem - 1)) + np.array([1, 3, 2]))
            for inode in range(n_node_elem):
                frame_of_reference_delta[we + ielem, inode, :] = [-1.0, 0.0, 0.0]
        elem_stiffness[we:we + n_elem_section_winglet] = 5
        elem_mass[we:we + n_elem_section_winglet] = 5
        boundary_conditions[wn] = 1
        boundary_conditions[wn + n_node_section_winglet - 1] = -1
        we += n_elem_section_winglet
        wn += n_node_section_winglet
        end_nodesR[3] = wn - 1 # potentially should leave this as end_nodesR[2] = wn-1?
        ...
        beam_number[we:we + n_elem_section_dihedral] = 6
        body_number[we:we + n_elem_section_dihedral] = 0
        structural_twist[we:we + n_elem_section_dihedral, :] = -in_structural_twist
        y[wn:wn + n_node_section_dihedral - 1] = y[wn - 1] + np.linspace(0.0, -np.cos(dihedral_outer)*span_inner_winglet, n_node_section_dihedral)[1:]
        z[wn:wn + n_node_section_dihedral - 1] = z[wn - 1] + np.linspace(0.0, np.sin(dihedral_outer)*span_inner_winglet, n_node_section_dihedral)[1:]
        for ielem in range(n_elem_section_dihedral):
            conn[we + ielem, :] = ((np.ones((3, ))*(we + ielem)*(n_node_elem - 1)) + np.array([1, 3, 2]))
            for inode in range(n_node_elem):
                frame_of_reference_delta[we + ielem, inode, :] = [1.0, 0.0, 0.0]
        ...
        #SECTION HINGED WINGLET L

        beam_number[we:we + n_elem_section_winglet] = 7
        body_number[we:we + n_elem_section_winglet] = 2
        structural_twist[we:we + n_elem_section_winglet, :] = -in_structural_twist
        y[wn:wn + n_node_section_winglet] = y[wn - 1] + np.linspace(0.0, -np.cos(dihedral_outer)*span_hinged_winglet, n_node_section_winglet)
        z[wn:wn + n_node_section_winglet] = z[wn - 1] + np.linspace(0.0, np.sin(dihedral_outer)*span_hinged_winglet, n_node_section_winglet)
        for ielem in range(n_elem_section_winglet):
            conn[we + ielem, :] = ((np.ones((3, ))*(we + ielem)*(n_node_elem - 1)) + np.array([2, 4, 3]))
            for inode in range(n_node_elem):
                frame_of_reference_delta[we + ielem, inode, :] = [1.0, 0.0, 0.0]
        elem_stiffness[we:we + n_elem_section_winglet] = 2
        elem_mass[we:we + n_elem_section_winglet] = 2
        boundary_conditions[wn] = 1
        boundary_conditions[wn + n_node_section_winglet - 1] = -1
        we += n_elem_section_winglet
        wn += n_node_section_winglet
        ...
        # centre tail
        beam_number[we:we + n_elem_centre_tail] = 8
        body_number[we:we + n_elem_centre_tail] = 0
        tail_beam_numbersC[0] = 7
        ...
        if vertical_tail:
            beam_number[we:we + n_elem_tail] = 9
            body_number[we:we + n_elem_tail] = 0
            tail_beam_numbersC[1] = 9
        ...
        # outer tail 0R
        beam_number[we:we + n_elem_outer_tail] = 11
        body_number[we:we + n_elem_outer_tail] = 0
        tail_beam_numbersR[0,0] = 11
        ...
        beam_number[we:we + n_elem_tail] = 12
        body_number[we:we + n_elem_tail] = 0
        tail_beam_numbersR[0,1] = 12
        ...
        # outer tail 1R
        beam_number[we:we + n_elem_outer_tail] = 14
        body_number[we:we + n_elem_outer_tail] = 0
        tail_beam_numbersR[1,0] = 14
        x[wn:wn + n_node_outer_tail - 1] = np.linspace(0.0, length_outer_tail, n_node_outer_tail)[1:]
        ...
        beam_number[we:we + n_elem_tail] = 15
        body_number[we:we + n_elem_tail] = 0
        tail_beam_numbersR[1,1] = 15
        ...
        # outer tail 0L
        beam_number[we:we + n_elem_outer_tail] = 17
        body_number[we:we + n_elem_outer_tail] = 0
        tail_beam_numbersL[0,0] = 17
        x[wn:wn + n_node_outer_tail - 1] = np.linspace(0.0, length_outer_tail, n_node_outer_tail)[1:]
        ...
        beam_number[we:we + n_elem_tail] = 18
        body_number[we:we + n_elem_tail] = 0
        tail_beam_numbersL[0,1] = 18
        ...
        # outer tail 1L
        beam_number[we:we + n_elem_outer_tail] = 20
        body_number[we:we + n_elem_outer_tail] = 0
        tail_beam_numbersL[1,0] = 20
        x[wn:wn + n_node_outer_tail - 1] = np.linspace(0.0, length_outer_tail, n_node_outer_tail)[1:]
        ...
        beam_number[we:we + n_elem_tail] = 21
        body_number[we:we + n_elem_tail] = 0
        tail_beam_numbersL[1,1] = 21
        ...
        beam_number[we:we + n_elem_tail] = 22
        body_number[we:we + n_elem_tail] = 0
        tail_beam_numbersL[1,2] = 22
        x[wn:wn + n_node_tail - 1] = x[end_tails_nodesL[1]]
        y[wn:wn + n_node_tail - 1] = y[end_tails_nodesL[1]] + np.linspace(0.0, -span_tail, n_node_tail)[1:]
        for ielem in range(n_elem_tail):
            conn[we + ielem, :] = ((np.ones((3, ))*(we + ielem)*(n_node_elem - 1)) + np.array([2, 4, 3]))
        ...
        # body_number[we:we + n_elem_fin] = 0
        # fin_beam_numberC = 23
        ...
        # left one
        beam_number[we:we + n_elem_fin] = 29
        body_number[we:we + n_elem_fin] = 0
        vfin_beam_numberL = 29
        ...
        # right one
        beam_number[we:we + n_elem_fin] = 30
        body_number[we:we + n_elem_fin] = 0
        vfin_beam_numberR = 30
    ...
    def generate_multibody_file():
        global x, y, z, n_node_section, n_node_section_dihedral, alpha, hinge_deg
        ######## MULTIBODY ########
        ...
        #LC2.node_in_body = n_node_section + n_node_section + n_node_section_dihedral + 1
        LC2.node_in_body = 18
        LC2.body = 0
        LC2.body_FoR = 1
        #LC2.scalingFactor = 1e8
        LC2.scalingFactor = 1e0
        ...
        #LC4.node_in_body = (n_node_section + n_node_section + n_node_section_dihedral)*2
        LC4.node_in_body = 36
        LC4.body = 0
        LC4.body_FoR = 2
        #LC4.scalingFactor = 1e8
        LC4.scalingFactor = 1e0
        ...
        MB2.body_number = 1
        # MB2.FoR_position = np.array([x[n_node_section + n_node_section + n_node_section_dihedral +1], 
        #                              y[n_node_section + n_node_section + n_node_section_dihedral+1], 
        #                              z[n_node_section + n_node_section + n_node_section_dihedral+1], 
        #                              0.0, 0.0, 0.0])
        MB2.FoR_position = np.array([x[18 +1], 
                                     y[18+1], 
                                     z[18+1], 
                                     0.0, 0.0, 0.0])
        
        print(x[19], y[19], z[19])
        print(x[52], y[52], z[52])
        ...
        MB4.body_number = 2
        # MB4.FoR_position = np.array([x[(n_node_section + n_node_section + n_node_section_dihedral)*2 + n_node_section_winglet + 2], 
        #                              y[(n_node_section + n_node_section + n_node_section_dihedral)*2 + n_node_section_winglet + 2], 
        #                              z[(n_node_section + n_node_section + n_node_section_dihedral)*2 + n_node_section_winglet + 2], 
        #                              0.0, 0.0, 0.0])
        MB4.FoR_position = np.array([x[52], 
                                     y[52], 
                                     z[52], 
                                     0.0, 0.0, 0.0])
    ...
    def generate_aero_file():
        ...
        # right wing (surface 0, beams 0, 1, 2)
        type = 'inboard'
        main_chord = aero_data[type]['chord']
        initial_node = 0
        final_node = end_nodesR[2]
        initial_elem = 0
        final_elem = end_elementsR[2]
        i_surf = 0
        ...
        # right hinged winglet (surface 1, beam 3)
        initial_node = end_nodesR[2]+1
        final_node = end_nodesR[-1]
        initial_elem = end_elementsR[2]+1
        ...
        # for i_elem in range(end_elementsR[1] + 1, end_elementsR[2] + 1):
        #     if i_elem == 0 or i_elem == end_elementsR[2]:
        #         continue                      #no control surface on the hinged winglet
        #     control_surface[i_elem, :] = 1

        # left wing (surface 2, beams 4, 5, 6)
        initial_node = end_nodesR[-1] + 1
        final_node = end_nodesL[2]
        initial_elem = end_elementsR[-1] + 1
        final_elem = end_elementsL[2]
        i_surf += 1
        airfoil_distribution[initial_elem: final_elem + 1, :] = aero_data['airfoil_indices'][aero_data[type]['airfoil']]
        surface_distribution[initial_elem: final_elem + 1] = i_surf
        surface_m[i_surf] = m
        ...
        # left hinged winglet (surface 3, beam 7)
        initial_node = end_nodesL[2] + 1
        final_node = end_nodesL[-1]
        initial_elem = end_elementsL[2] + 1
        ...
        #### NOTE: Sweep
        sweep[8,1] = -np.deg2rad(hinge_deg)
        sweep[9,0] = -np.deg2rad(hinge_deg)
        sweep[24,1] = np.deg2rad(hinge_deg)
        sweep[25,0] = np.deg2rad(hinge_deg)
    ...
    def generate_solver_file():
        ...
        settings['SHARPy'] = {'case': case_name,
                              ...
                              'write_screen': 'on',
                              'write_log': 'on',
                              'log_folder': route + '/output/' + outputfolderpath,
        ...
        settings['NonLinearDynamicMultibody'] = {'min_delta': 1e-3,
                                                  ...
                                                  'time_integrator_settings': {'newmark_damp': 1, # original: 'newmark_damp': 0.0001,
                                                                                'dt': dt},
                                                  'gravity_on': gravity,
                                                  'gravity': 9.81,
                                                  'num_steps': n_tstep,
        ...
        settings['StepUvlm'] = {'print_info': 'off',
                                'num_cores': num_cores,
                                'convection_scheme': 3, #2 for perfectly convected wake, 3 for free wake
                                ...
                                'dt': dt,
                                'cfl1': cfl1}
        #settings['NoAero'] = {'dt': dt}
        settings['DynamicCoupled'] = {'structural_solver': 'NonLinearDynamicMultibody',
                                      'structural_solver_settings': settings['NonLinearDynamicMultibody'],
                                      ...
                                      'n_time_steps': n_tstep,
                                      'dt': dt,
                                      'include_unsteady_force_contribution': 'on', #might make solution more stable if this is turned off
                                    #   'cleanup_previous_solution': 'on',
                                      ...
                                      'postprocessors_settings': {'BeamLoads': {'csv_output': 'on'}, # you need to specify the dict even if empty
                                                                  'SaveData': {},
                                                                  ...
                                                                  'AerogridPlot': {
                                                                      ...
                                                                      'include_applied_forces': 'on',
                                                                      'minus_m_star': 0},
                                                                  #'CreateSnapshot': {'frequency': 3, 'keep': 2},
                                                                      # every how many tsteps if should create a snapshot
                                                                                     #'frequency': 50,}
                                                                  ...
                                                                  'WriteVariablesTime': {
                                                                      ...
                                                                      #'structure_nodes': np.arange(64),
                                                                      'structure_nodes': [0,32],
                                                                      'structure_variables': ['pos','pos_dot'],
                                                                      'FoR_variables': ['quat', 'for_pos', 'for_vel', 'for_acc','forces_constraints_FoR'],
    ...
    # extract information root loads
    ...
    route_test_dir = route

    route_export = route_test_dir + '/output/output_data_hxhale'
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/root_gusti{:04g}_gustl{:04g}_flare{:04g}.txt'.format(gust_intensity * 100, gust_length, hinge_deg * 100)
    ...
    for it in range(n_tsteps):
        psiH = case_data.structure.timestep_info[it].psi[9, 0]
        psiB = case_data.structure.timestep_info[it].psi[8, 1]
        theta1[it] = algebra.quat2euler(algebra.rotation2quat(algebra.multiply_matrices((algebra.crv2rotation(psiH).T),algebra.crv2rotation(psiB))))[1]

    route_export = route_test_dir + '/output/output_data_hxhale'
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/theta1_gusti{:04g}_gustl{:04g}_flare{:04g}.txt'.format(gust_intensity * 100, gust_length, hinge_deg * 100)
    ...
    for it in range(n_tsteps):
        psiH = case_data.structure.timestep_info[it].psi[25, 0]
        psiB = case_data.structure.timestep_info[it].psi[24, 1]
        theta2[it] = algebra.quat2euler(algebra.rotation2quat(algebra.multiply_matrices((algebra.crv2rotation(psiH).T),algebra.crv2rotation(psiB))))[1]

    route_export = route_test_dir + '/output/output_data_hxhale'
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/theta2_gusti{:04g}_gustl{:04g}_flare{:04g}.txt'.format(gust_intensity * 100, gust_length, hinge_deg * 100)
    ...
    for it in range(n_tsteps):
        #cga = algebra.euler2rot([0, case_data.trimmed_values[0], 0])
        cga = algebra.euler2rot([0, alpha, 0])
        pos = np.dot(cga, case_data.structure.timestep_info[it].for_pos[0:3])
        # pos = np.dot(case_data.structure.timestep_info[it].cga(), case_data.structure.timestep_info[it].for_pos[0:3])
        root[it] = pos

    route_export = route_test_dir + '/output/output_data_hxhale'
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/rootxyz_gusti{:04g}_gustl{:04g}_flare{:04g}.txt'.format(gust_intensity * 100, gust_length, hinge_deg * 100)
    np.savetxt(dest_file, root)
    print('Saved rootxyz array to {}'.format(dest_file))

    # extract information hinge1xyz
    n_tsteps = len(case_data.structure.timestep_info)
    hinge1 = np.zeros((n_tsteps, 3))
    for it in range(n_tsteps):
        # pos = np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].mb_quat[1, :]), case_data.structure.timestep_info[it].mb_FoR_pos[1, 0:3])
        # cga = algebra.euler2rot([0, case_data.trimmed_values[0], 0])
        cga = algebra.euler2rot([0, alpha, 0])
        pos = np.dot(cga, case_data.structure.timestep_info[it].for_pos[0:3]) + np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].quat), case_data.structure.timestep_info[it].pos[18, 0:3])
        # pos = np.dot(case_data.structure.timestep_info[it].cga(), case_data.structure.timestep_info[it].for_pos[0:3]) + np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].quat), case_data.structure.timestep_info[it].pos[12, 0:3])
        hinge1[it] = pos

    route_export = route_test_dir + '/output/output_data_hxhale'
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/hinge1xyz_gusti{:04g}_gustl{:04g}_flare{:04g}.txt'.format(gust_intensity * 100, gust_length, hinge_deg * 100)
    np.savetxt(dest_file, hinge1)
    print('Saved hinge1xyz array to {}'.format(dest_file))
 
    # extract information hinge2xyz
    n_tsteps = len(case_data.structure.timestep_info)
    hinge2 = np.zeros((n_tsteps, 3))
    for it in range(n_tsteps):
        # pos = np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].mb_quat[2, :]), case_data.structure.timestep_info[it].mb_FoR_pos[2, 0:3])
        # cga = algebra.euler2rot([0, case_data.trimmed_values[0], 0])
        cga = algebra.euler2rot([0, alpha, 0])
        pos = np.dot(cga, case_data.structure.timestep_info[it].for_pos[0:3]) + np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].quat), case_data.structure.timestep_info[it].pos[51, 0:3])
        # pos = np.dot(case_data.structure.timestep_info[it].cga(), case_data.structure.timestep_info[it].for_pos[0:3]) + np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].quat), case_data.structure.timestep_info[it].pos[29, 0:3])
        hinge2[it] = pos

    route_export = route_test_dir + '/output/output_data_hxhale'
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/hinge2xyz_gusti{:04g}_gustl{:04g}_flare{:04g}.txt'.format(gust_intensity * 100, gust_length, hinge_deg * 100)
    np.savetxt(dest_file, hinge2)
    print('Saved hinge2xyz array to {}'.format(dest_file))   
    ...
    for it in range(n_tsteps):
        # cga = algebra.euler2rot([0, case_data.trimmed_values[0], 0])
        cga = algebra.euler2rot([0, alpha, 0])
        pos = np.dot(cga, case_data.structure.timestep_info[it].for_pos[0:3]) + np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].quat), case_data.structure.timestep_info[it].pos[33, 0:3])
        # pos = np.dot(case_data.structure.timestep_info[it].cga(), case_data.structure.timestep_info[it].for_pos[0:3]) + np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].quat), case_data.structure.timestep_info[it].pos[17, 0:3])
        tip1[it] = pos

    route_export = route_test_dir + '/output/output_data_hxhale'
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/tip1xyz_gusti{:04g}_gustl{:04g}_flare{:04g}.txt'.format(gust_intensity * 100, gust_length, hinge_deg * 100)
    ...
    for it in range(n_tsteps):
        # cga = algebra.euler2rot([0, case_data.trimmed_values[0], 0])
        cga = algebra.euler2rot([0, alpha, 0])
        pos = np.dot(cga, case_data.structure.timestep_info[it].for_pos[0:3]) + np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].quat), case_data.structure.timestep_info[it].pos[66, 0:3])
        # pos = np.dot(case_data.structure.timestep_info[it].cga(), case_data.structure.timestep_info[it].for_pos[0:3]) + np.dot(algebra.quat2rotation(case_data.structure.timestep_info[it].quat), case_data.structure.timestep_info[it].pos[34, 0:3])
        tip2[it] = pos

    route_export = route_test_dir + '/output/output_data_hxhale'
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/tip2xyz_gusti{:04g}_gustl{:04g}_flare{:04g}.txt'.format(gust_intensity * 100, gust_length, hinge_deg * 100)
