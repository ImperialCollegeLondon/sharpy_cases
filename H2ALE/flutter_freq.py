...
# def generate_fem():
    ...
    body_number[we:we + n_elem_main1] = 0
    ...
    boundary_conditions[wn + n_node_main1 - 1] = -1
    ...
    body_number[we:we + n_elem_main1] = 1    
    y[wn:wn + n_node_main2 ] = y[wn - 1] + np.linspace(0.0, np.cos(lambda_dihedral) * span_main2, n_node_main2)
    z[wn:wn + n_node_main2 ] = z[wn - 1] + np.linspace(0.0, np.sin(lambda_dihedral) * span_main2, n_node_main2)
    ...
    boundary_conditions[wn + n_node_main2 - 1] = -1
    we += n_elem_main2
    wn += n_node_main2 

    # inner left wing
    beam_number[we:we + n_elem_main1] = 2
    body_number[we:we + n_elem_main1] = 0    
    ...
    elem_stiffness[we:we + n_elem_main1] = 0
    elem_mass[we:we + n_elem_main1] = 0
    boundary_conditions[wn + n_node_main1 - 2] = -1
    ...
    # outer left wing
    beam_number[we:we + n_elem_main2] = 3
    body_number[we:we + n_elem_main2] = 2
    y[wn:wn + n_node_main2 ] = y[wn - 1] + np.linspace(0.0, -np.cos(lambda_dihedral) * span_main2, n_node_main2)
    z[wn:wn + n_node_main2 ] = z[wn - 1] + np.linspace(0.0, np.sin(lambda_dihedral) * span_main2, n_node_main2)
    for ielem in range(n_elem_main2):
        conn[we + ielem, :] = ((np.ones((3,)) * (we + ielem) * (n_node_elem - 1)) +
    ...
    # fuselage
    beam_number[we:we + n_elem_fuselage] = 4
    body_number[we:we + n_elem_fuselage] = 0
    ...
    # fin
    beam_number[we:we + n_elem_fin] = 5
    ...
    # right tail
    beam_number[we:we + n_elem_tail] = 6
...
# def generate_aero_file():
    ...
    we = 0
    wn = 0
    # right inner wing (surface 0, beam 0)
    i_surf = 0
    airfoil_distribution[we:we + n_elem_main1, :] = 0
    surface_distribution[we:we + n_elem_main1] = i_surf
    surface_m[i_surf] = m
    aero_node[wn:wn + n_node_main1] = True
    temp_chord = np.linspace(chord_main, chord_main, n_node_main1)
    temp_sweep = np.linspace(0.0, 0 * np.pi / 180, n_node_main1)
    node_counter = 0
    for i_elem in range(we, we + n_elem_main1):
    ...
    # right outer wing (surface 1, beam 1)
    i_surf = 1
    airfoil_distribution[we:we + n_elem_main2, :] = 0
    surface_distribution[we:we + n_elem_main2] = i_surf
    surface_m[i_surf] = m
    aero_node[wn:wn + n_node_main2] = True
    temp_chord = np.linspace(chord_main, chord_main, n_node_main2)
    temp_sweep = np.linspace(0.0, 0 * np.pi / 180, n_node_main2)
    node_counter = 0
    for i_elem in range(we, we + n_elem_main2):
        for i_local_node in range(n_node_elem):
            if not i_local_node == 0:
                node_counter += 1
            chord[i_elem, i_local_node] = temp_chord[node_counter]
    ...
    # left inner wing (surface 0, beam 0)
    i_surf = 2
    airfoil_distribution[we:we + n_elem_main1, :] = 0
    # airfoil_distribution[wn:wn + n_node_main - 1] = 0
    surface_distribution[we:we + n_elem_main1] = i_surf
    surface_m[i_surf] = m
    aero_node[wn:wn + n_node_main1 - 1] = True
    ...
    # left outer wing (surface 2, beam 2)
    i_surf = 3
    airfoil_distribution[we:we + n_elem_main2, :] = 0
    surface_distribution[we:we + n_elem_main2] = i_surf
    surface_m[i_surf] = m
    ...
    #### NOTE: Sweep
    sweep[5,1] = -np.deg2rad(hinge_deg)
    sweep[6,0] = -np.deg2rad(hinge_deg)
    sweep[13,1] = np.deg2rad(hinge_deg)
    sweep[14,0] = np.deg2rad(hinge_deg)
...
# def generate_solver_file():
    ...
    settings['DynamicCoupled'] = {'structural_solver': solver,
                                ...
                                'postprocessors_settings': {'BeamLoads': {'csv_output': 'on'},
                                                            ...
                                                            'AerogridPlot': {
                                                                ...
                                                                'stride': 10},
                                                            'WriteVariablesTime' : {
                                                                'structure_variables': ['for_pos', 'pos','pos_dot', 'pos_ddot', 'psi','psi_dot', 'psi_ddot','psi_local','psi_dot_local','q', 'dqdt','dqddt'],
                                                                'FoR_variables': ['mb_quat','mb_dquatdt','in_global_AFoR','mb_FoR_vel', 'mb_FoR_acc', 'mb_FoR_pos'] ,
                                                                'FoR_number': np.array([0,1,2], dtype=int),
                                                                'structure_nodes': list(range(0, 66))
                                                            }
                                                            }}
    ...
    for it in range(n_tsteps):
        psiH = case_data.structure.timestep_info[it].psi[6, 0]
    ...
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/theta1_gusti{:04g}_flare{:04g}.txt'.format(gust_length * 100, hinge_deg * 100)
    ...
    for it in range(n_tsteps):
        psiH = case_data.structure.timestep_info[it].psi[14, 0]
        psiB = case_data.structure.timestep_info[it].psi[13, 1]
    ...
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/theta2_gusti{:04g}_flare{:04g}.txt'.format(gust_length * 100, hinge_deg * 100)
    ...
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/hinge1xyz_gusti{:04g}_flare{:04g}.txt'.format(gust_length * 100, hinge_deg * 100)
    ...
    if not os.path.exists(route_export):
        os.makedirs(route_export)  
    dest_file = route_export + '/hinge2xyz_gusti{:04g}_flare{:04g}.txt'.format(gust_length * 100, hinge_deg * 100)
